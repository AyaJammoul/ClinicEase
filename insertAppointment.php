...
function getPatientId($conn, $patientName, $mobileNumber) {
    $stmt = $conn->prepare("SELECT patientid FROM Patientreg WHERE patientname = ? AND mobilenumber = ?");
    $stmt->bind_param("ss", $patientName, $mobileNumber);
    $stmt->execute();
    $result = $stmt->get_result();
    return $result->num_rows > 0 ? $result->fetch_assoc()['patientid'] : null;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $vitalFill = $_POST['vitalfill'];

    $patientId = getPatientId($conn, $patientName, $mobileNumber);
    if ($patientId === null) {
        echo "Invalid patient name.";
        exit();
    }
    ...
    if (isset($_POST['appointmenttime'])) {
        ...
        $stmt->bind_param("iisssissss", $patientId, $doctorId, $clinicName, $appointmentDate, $procedureName, $procedureDuration, $appointmentTime, $getdate, $status, $vitalFill);
